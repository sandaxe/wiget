...
const fs = require('fs');
const packageJsonData = JSON.parse(fs.readFileSync(paths.appPackageJson, {encoding: "UTF-8"}));
const vendorDependencies = Object.keys(packageJsonData.dependencies);
...
const fillterCss = function(modulePath) {
  if(isWindows) {
...
const prdConfig = {
    ...
    rules: [
      ...
      // It's important to do this before Babel processes the JS.
      {
        ...
        exclude: function(modulePath) {
          return /shared\/libs/.test(modulePath) || 
          /shared\\libs/.test(modulePath)
        }
      },
      {
  ...
  plugins: [
    ...
    new Critters(),
    new InterpolateHtmlPlugin(env.raw),
    //preload the asserts (css / js / fonts) for faster performance
    //one caveats could be user will get the assert that they don't need. right it is ok
    // new PreloadWebpackPlugin({
    //   rel: 'prefetch',
    //   include: 'asyncChunks', // asyncChunks / initial / allChunks / allAssets
    //   fileBlacklist: [
    //     /\.(woff|ttf|eot)$/,
    //     /react-intl/, /studio/, /workflow/,
    //     /permission/, /locales/, /static\/js\/form\/form.component/
    //   ]
    // }),
    // new PreloadWebpackPlugin({
    //   rel: 'preload',
    //   include: 'allAssets', // asyncChunks / initial / allChunks / allAssets
    //   as(entry) {
    ...
    //   // index.html - to re-fresh new resource on every new build
...
//set default splitChunk option when running profile
if(process.env.PROFILE) {
  delete prdConfig.optimization.splitChunks;
}
